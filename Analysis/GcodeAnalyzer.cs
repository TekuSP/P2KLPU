...
static class GcodeAnalyzer
{
    ...
    public static GcodeAnalysis Analyze(string[] lines, Options options)
    {
        ...
        if (options.RawMmuMode)
        {
            ...
            // Custom tower (TOWER) mode: surface planning findings, and analyze the timeline
            // WITH generated purge/sustain so the splice/ping plan matches the rewritten file.
            TowerPlanStats? towerStats = null;
            if (options.TowerMode)
            {
                var towerPlan = TowerPlanner.Plan(lines, scan, options);
                errors.AddRange(towerPlan.Errors);
                warnings.AddRange(towerPlan.Warnings);
                if (towerPlan.Errors.Count == 0 && towerPlan.Injections.Count > 0)
                    scan = RawMmuScanner.Scan(lines, options, towerPlan.Injections);
                towerStats = towerPlan.Stats;
            }
            ...
            // In TOWER mode the tower is generated by P2KLPU itself; slicer tower detection is irrelevant.
            if (!options.TowerMode)
            {
    ...
    private static void AddPrusaSetupFindings(string[] lines, Options options, bool sawAnyToolchange, List<string> warnings, List<string> errors)
    {
        ...
        // In TOWER mode a disabled slicer wipe tower is the REQUIRED configuration.
        if (!options.TowerMode && sawAnyToolchange && SlicerConfigDetector.TryReadPrusaInt(lines, "wipe_tower") == 0)
        {
            warnings.Add("PrusaSlicer wipe tower is disabled but the print has toolchanges. Transitions will purge into the model — enable the wipe tower, or enable P2KLPU's own tower with ';P2KLPU TOWER=1'.");
        }
