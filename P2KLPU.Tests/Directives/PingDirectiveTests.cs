...
public sealed class PingDirectiveTests
{
    [Fact]
    public void ApplyTo_LinearPingLength_SetsFixedIntervalAndDisablesGrowth()
    {
        var lines = new[]
        {
        ...
        Assert.Equal(1000, updated.PingInitialIntervalMm);
        Assert.Equal(3000, updated.PingMaxIntervalMm);
        Assert.Equal(1.0, updated.PingLengthMultiplier);
    }

    [Fact]
    public void ApplyTo_LinearPingLength_UnderscoreAliasAlsoWorks()
    {
        var lines = new[]
        {
            ";P2KLPU LINEAR_PING_LENGTH=750.5",
        };
        ...
        Assert.Equal(750.5, updated.PingInitialIntervalMm);
        Assert.Equal(3000, updated.PingMaxIntervalMm);
        Assert.Equal(1.0, updated.PingLengthMultiplier);
    }
