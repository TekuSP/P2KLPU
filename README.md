...
TOWER directives: `TOWER=0|1`, `TOWER_X`, `TOWER_Y`, `TOWER_WIDTH`, `TOWER_DEPTH`, `TOWER_BRIM_LOOPS=4`, `TOWER_SPEED=<mm/min>` (default: profile), `TOWER_FIRST_LAYER_SPEED=<mm/min>` (default: profile), `TOWER_SUSTAIN_PERIMETERS=2`, `TOWER_SUSTAIN_SPACING=6`, `TOWER_SUSTAIN_LATTICE_LAYERS` (default: all layers below a purge), `TOWER_MAX_FLOW=<mm³/s>` (default: profile cap), `TOWER_SPLICE_DWELL=0` (ms), `TOWER_EXTRUSION_WIDTH` (default: footer `extrusion_width`, else 0.45), `PURGE_DEFAULT`, `PURGE_<FROM>_<TO>`.
