...
Setup:
1. In PrusaSlicer, **disable the wipe tower** (Print Settings → Multiple extruders → Wipe tower OFF). TOWER mode errors if the export still contains the slicer's tower (you'd get two towers). Purging into infill or objects is done by P2KLPU itself (see below), no slicer tower needed. If you prefer PrusaSlicer's own wipe-into-infill planning, keep its tower on and add `;P2KLPU TOWER_REPLACE_SLICER_TOWER=1`: P2KLPU then removes the slicer's tower from the file (its purge lines, perimeters, sparse grids, the travels to it and its pressure-advance disable) and honors what PrusaSlicer wiped into the model.
...
- Grows **layer-synchronized with the print** (PrusaSlicer-style) and **stops at the last toolchange**: every layer up to and including the last color change's layer gets a tower pass — "sustaining" layers (walls + a sparse support lattice) on layers without a toolchange, perimeter + zigzag fill consumed up to the purge target on layers with one (full if needed). Nothing is printed above the last color change. The tower top is therefore always at the nozzle's current Z, so tower visits never move the toolhead down (only the usual hop); no P2PP-style lagging Z. Fill orientation alternates 90° per layer for anchoring.
- Sustaining passes are the price of that: a print whose color changes all sit high up (a two-color Benchy with the changes in the roof) spends a pass on every layer below them. **The support lattice is printed only where something above lands on it**: every layer that has a purge somewhere above it keeps the lattice (so the whole tower below the last color change is latticed, and only the top purge layer's unused remainder is left bare). `TOWER_SUSTAIN_LATTICE_LAYERS=<n>` restricts the lattice to the n sustaining layers directly below each purge layer and prints walls only further down — cheaper (`n=1` leaves just the walls on a long single-color stretch, `TOWER_SUSTAIN_PERIMETERS=1` halves that again), but the lattice under a purge then bridges the hollow tower wall to wall, which PETG or a wide tower may sag on; `n=2` gives it a lattice of its own to rest on.
- **No mid-air purge**: the lattice under a purge layer (`TOWER_SUSTAIN_SPACING`, default 6mm; 0 = walls only) uses the same orientation parity as the dense fill — so the purge fill always finds perpendicular support lines one layer below and bridges at most one lattice gap, never the whole footprint.
- **Adaptive lattice** (the tower's adaptive cubic, on by default): the surface that needs support is the next purge layer above, so the lattice keeps the dense spacing within `TOWER_SUSTAIN_DENSE` mm of height below it (default = the spacing, 6 mm), doubles to 12 mm for the zone below that (12 mm tall), and 24 mm beyond (`TOWER_SUSTAIN_SPACING_MAX`, default 4× the spacing). Deep inside the tower a lattice line bridges up to 24 mm between the perpendicular lines under it, which is harmless there. `TOWER_SUSTAIN_ADAPTIVE=0` keeps one spacing everywhere.
...
Speeds: by default the tower prints at **the sliced profile's own speeds** — normal layers at the solid-infill speed (falling back to infill, then perimeter speed), the first layer and the calibration squares at `first_layer_speed` (percentages resolved the way PrusaSlicer does), and the flow cap taken from the smallest `max_volumetric_speed` / `filament_max_volumetric_speed` in the profile (none set = uncapped, like the rest of the print). Give `TOWER_SPEED`, `TOWER_FIRST_LAYER_SPEED` or `TOWER_MAX_FLOW` a number to override any one of them; `PRINT` on any of them selects the profile value explicitly. The console reports what was taken from the profile.

TOWER directives: `TOWER=0|1`, `TOWER_X`, `TOWER_Y`, `TOWER_WIDTH`, `TOWER_DEPTH`, `TOWER_BRIM_LOOPS=4`, `TOWER_SPEED=<mm/min>` (default: profile), `TOWER_FIRST_LAYER_SPEED=<mm/min>` (default: profile), `TOWER_SUSTAIN_PERIMETERS=2`, `TOWER_SUSTAIN_SPACING=6`, `TOWER_SUSTAIN_LATTICE_LAYERS` (default: all layers below a purge), `TOWER_SUSTAIN_ADAPTIVE=1`, `TOWER_SUSTAIN_DENSE=6`, `TOWER_SUSTAIN_SPACING_MAX=24`, `TOWER_MAX_FLOW=<mm³/s>` (default: profile cap), `TOWER_SPLICE_DWELL=0` (ms), `TOWER_EXTRUSION_WIDTH` (default: footer `extrusion_width`, else 0.45), `PURGE_DEFAULT`, `PURGE_<FROM>_<TO>`, `PURGE_INTO_INFILL=0|1`, `PURGE_INTO_OBJECT=<name>`, `PURGE_JUNCTION=MODEL|TOWER` (default MODEL), `TOWER_REPLACE_SLICER_TOWER=0|1`.

#### Purging into the model: wipe into infill / wipe into object

Internal infill is invisible, so it can be printed in whatever comes out of the nozzle right after a color change. `;P2KLPU PURGE_INTO_INFILL=1` makes P2KLPU do that itself, with no slicer tower involved: for every toolchange it takes the `;TYPE:Internal infill` blocks that PrusaSlicer would print later on the same layer (after the change, before the next one), in stream order, until their filament covers the transition's pair purge, and prints them inside the purge visit instead — travel there, prime, the block's own lines verbatim (same coordinates and speeds, the object's Klipper label re-applied so cancelling the object still works), retract, back to where the toolchange left the head. Their sliced position and the travel run that led into them are dropped (the block's `M204` is replayed in the visit; the retract/unretract lines around the hole are kept or dropped so the extruder state in the file stays balanced). Blocks under 0.5 mm of filament stay where they are: a visit's travel, prime and retract cost more than they purge. Perimeters, solid and top infill are never touched. `;P2KLPU PURGE_INTO_OBJECT=<Klipper object name>` (repeatable, comma-separated) does the same with everything of a sacrificial object — its colors will mix.

The tower then only takes what the model cannot: `tower purge = max(pair purge − into model, MINSPLICE floor, junction floor)`. The junction floor is `SPLICEOFFSET + 15` mm of purge span in front of the declared color change; by default (`PURGE_JUNCTION=MODEL`) the relocated infill/object counts toward it, so a transition whose model purge covers the whole tail needs no tower visit, and when that holds for every transition **no tower is generated at all** — the model is the purge system. `PURGE_JUNCTION=TOWER` keeps those `SPLICEOFFSET + 15` mm on the tower instead (the color change stays visible on the tower, the model carries the tail). The footprint is sized from the remaining per-layer demand, so the more the model takes, the smaller the tower. The console and footer report the transitions that purge into the model, the mm they take, and the tower purge saved.

Alternative, `;P2KLPU TOWER_REPLACE_SLICER_TOWER=1`: let PrusaSlicer plan the wiping ("Wipe into this object's infill" / "Wipe into this object" in the object's *Wipe options*; both exist only while the slicer's wipe tower is enabled, so keep it on). P2KLPU removes the slicer's tower from the file, measures what PrusaSlicer printed into the model after each toolchange (the extrusion up to its `; PURGING FINISHED` marker) and applies the same purge math. Set PrusaSlicer's **Purging volumes** at least as large as your P2KLPU purges in mm³ (mm × 2.405: `PURGE_DEFAULT=120` → 290 mm³, 180 → 435 mm³) so it assigns as much to the model as it can, and `filament_minimal_purge_on_wipe_tower` to 0. P2KLPU's tower goes where the slicer's stood if it fits. Side effect worth knowing: PrusaSlicer disables Klipper pressure advance for its tower (`SET_PRESSURE_ADVANCE ADVANCE=0` at every toolchange) and never restores it; those lines go with the tower, so the model keeps its pressure advance.
...
2. Add `;P2KLPU CALIBRATE_OFFSET=20,20,8` to the directives (start, step, count; optional 4th value = test input, e.g. `20,20,8,2`, or `ALL` to cycle through every filament in the profile — DI1→DI2→DI3→DI4→DI1… — so each color pair's purge tail gets measured, not just the offset). Keep your other directives (MINSPLICE etc.). Optional: `;P2KLPU CALIBRATE_SCALE=1` adds a direct-reading scale beside every square (see below); it is off by default because the thin strokes are tedious to remove from the bed.
...
Reading it — any square where the color change is visible works, and all of them should agree. The number printed in front of a square is the offset that square was printed with, not the answer; the answer comes from where the color flips inside the fill:
1. Count old-color fill lines from the square's front edge up to the **first line showing any trace of the new color**: `measured = lines × mm-per-line`. Do not count to where the color is clean — that later point is the purge tail, often 60–100 mm further, and using it makes the offset far too small (the color then changes too early in real prints).
...
3. Count fill lines from the first trace until the color is fully clean: that tail in mm is your melt-zone purge requirement — set `PURGE_DEFAULT ≥ tail + 30`.

Cross-check without counting: squares whose declared offset is below the true value show the new color already in their **frame** (the change came before the purge started); the smallest declared offset whose frame is still entirely the old color is within one step of the answer.

The squares print at the profile's `first_layer_speed` and flow cap by default, like any first layer of yours. Keep it that way: the junction arrives earlier the more the Palette's buffer drains around a splice, and the buffer drains with consumption, so squares printed far slower than your prints would read a lower offset than the prints need. `TOWER_FIRST_LAYER_SPEED` and `TOWER_MAX_FLOW` override it if you want to test a specific speed.
...
With `;P2KLPU CALIBRATE_SCALE=1` a **scale is printed on the left of every square**, counting down from `declared + 10` at the front edge (a tick every 10 mm of filament, a number every 30 mm), so **the number beside the color change is your SPLICEOFFSET** with no counting. The columns are spaced wider to make room for it.
...
By default the tower runs at the profile's speeds and flow cap, no slower than the rest of the print. `TOWER_MAX_FLOW=<mm³/s>` can **cap tower extrusion volumetrically** regardless of layer height if the buffer collapses during splices (error 120): a cap around 1.8 mm³/s keeps buffer draw during a splice well inside what the Palette recovers between splices, and `TOWER_SPLICE_DWELL=3000` pauses 3 s at the start of every purge visit so the Palette gets a head start. Do not reach for the cap on a 121 that comes with an idle Palette — that is the device stalling in its own splice sequence, not the buffer being outrun. The analysis warns you when most transitions sit at the MINSPLICE floor (continuous-splicing territory). Raising `MINSPLICE`/purge lengths also lowers splice cadence at the cost of more purge.
...
- `SPLICEOFFSET` shifts that end like every other junction, so the filament that actually arrives after the last extrusion is `EXTRAENDFILAMENT + SPLICEOFFSET`, and it must cover the extruder-to-nozzle path (~100 mm on a direct-drive toolhead) because the extruder can only push while it still grips the filament. A `SPLICEOFFSET` set too low therefore does two things at once: colors change too early **and** the last part of the print runs dry (a Benchy printed at −25 with the true value near 60 lost its chimney top). The analysis warns when the tail comes out under 150 mm; 150 + a correct offset is enough, 250 gives margin for an offset that is still being tuned.
